...
<nav aria-label="Navegación móvil inferior" class="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border-t border-slate-200/30 dark:border-slate-800/30 px-4 pb-safe pt-3 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
    ...
    {#if currentEnvironment === 'STORE_MANAGER' && isDashboard}
        <div class="absolute -top-10 left-1/2 -translate-x-1/2">
            <button 
                ...
                class="bg-blue-600 text-white p-4 rounded-2xl shadow-2xl shadow-blue-600/40 active:scale-90 transition-all border-4 border-slate-50 dark:border-slate-950 group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
            >
