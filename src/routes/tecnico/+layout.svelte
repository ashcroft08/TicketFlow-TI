<script lang="ts">
    import { LayoutDashboard, Ticket, LogOut, Menu, X } from 'lucide-svelte';
...
<div class="flex min-h-screen bg-bg-main dark:bg-dark-bg-main text-text-main dark:text-dark-text-main transition-colors duration-500 overflow-x-hidden">
    ...
    <!-- Sidebar -->
    <aside class="fixed inset-y-0 left-0 w-64 glass-card !rounded-none border-r border-white/5 z-50 transform transition-transform duration-300 lg:relative lg:translate-x-0 {isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} overflow-hidden">
        <div class="h-full flex flex-col">
            <!-- Logo Area -->
            <div class="flex items-center justify-center py-8 shrink-0">
                <img src={logoClaro} alt="TicketFlow" class="h-8 w-auto block dark:hidden" />
            ...
            <nav class="flex-grow px-4 space-y-1 overflow-y-auto custom-scrollbar">
                <p class="text-[9px] font-black uppercase tracking-[0.2em] text-text-dim px-2 mb-2 opacity-50">Operaciones</p>
                <a 
                    href="/tecnico/dashboard"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {activePath.includes('/dashboard') ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-white/60 hover:bg-primary/10 hover:text-white'}"
                >
                    <LayoutDashboard class="w-4 h-4" />
                    <span class="text-[11px] font-black uppercase tracking-tight">Panel Técnico</span>
                </a>
            </nav>
            ...
            <div class="p-4 border-t border-white/5 space-y-4 shrink-0">
                ...
                <form action="/?/logout" method="POST" use:enhance>
                    <button type="submit" class="w-full flex items-center gap-3 px-3 py-2.5 text-error/70 hover:text-error hover:bg-error/10 rounded-lg transition-all font-bold text-[10px] uppercase tracking-widest border border-transparent hover:border-error/20">
