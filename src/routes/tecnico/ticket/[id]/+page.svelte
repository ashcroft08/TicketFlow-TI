            ...
            <div class="bg-white/70 dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200 dark:border-slate-700/60 rounded-[24px] shadow-sm overflow-hidden flex flex-col shrink-0">
                ...
                {#if isManagementExpanded}
                    ...
                    <button 
                        ...
                        class="flex-1 py-3 text-sm font-semibold flex items-center justify-center gap-2 transition-colors {activeDetailTab === 'info' ? 'border-b-2 border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}"
                    >
                        <FileText class="w-4 h-4" /> Info. del Problema
                    </button>
                </div>
                        ...
                        <form method="POST" action="?/updateDetails" class="flex flex-col gap-5" use:enhance={() => {
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <select name="id_estado" id="id_estado" class="h-10 px-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm">
                                ...
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <select name="id_nivel_atencion" id="id_nivel_atencion" class="h-10 px-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <select name="id_categoria" id="id_categoria" class="h-10 px-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm">
                                    <option value="">-- Clasificar Problema --</option>
                                    {#each data.categorias as categoria}
                                        <option value={categoria.id_categoria} selected={ticket.id_categoria === categoria.id_categoria}>{categoria.nombre}</option>
                                    {/each}
                                </select>
                            </div>
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <textarea name="notas_tecnico" id="notas_tecnico" rows="4" placeholder="Escribe el diagnóstico, pasos realizados, repuestos necesarios..." class="p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm custom-scrollbar">{ticket.notas_tecnico || ''}</textarea>
                            </div>

                            <button type="submit" disabled={isSubmittingDetails} class="mt-2 w-full h-11 bg-slate-800 dark:bg-slate-700 text-white font-semibold text-sm rounded-xl flex items-center justify-center gap-2 hover:bg-slate-700 dark:hover:bg-slate-600 transition-colors disabled:opacity-50">
                                {#if isSubmittingDetails}
            ...
            <div class="p-4 bg-white/50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700/60 rounded-b-[24px]">
                ...
                >
                    <input 
                        type="text" 
                        name="comentario" 
                        placeholder={ticket.id_usuario ? "Escribe un mensaje al encargado..." : "Debes tomar el ticket para chatear..."}
                        required
                        autocomplete="off"
                        ...
                        disabled={isSubmittingChat || !ticket.id_usuario}
                    />
                    <button 
                        type="submit"
                        disabled={isSubmittingChat || !ticket.id_usuario}
