...
{#if isOpen}
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6" transition:fade={{ duration: 200 }}>
        ...
        <div 
            class="relative w-full max-w-2xl h-[70vh] bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col" 
            role="dialog"
            aria-modal="true"
        ...
        >
            <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/20">
                <div class="flex items-center gap-3">
                    <History class="w-6 h-6 text-indigo-600 aria-hidden=true" />
                    <h2 id="history-modal-title" class="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tight">Trazabilidad del Activo</h2>
                </div>
            ...
            <div class="flex-grow overflow-y-auto p-8 space-y-6 custom-scrollbar">
                    ...
                    {#each movimientosActivo as mov}
                        <div transition:slide class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700/50 relative overflow-hidden group">
                            ...
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-[10px] font-bold text-slate-400 flex items-center gap-1.5"><Clock class="w-3 h-3 aria-hidden=true" /> {formatDate(mov.created_at)}</span>
                            </div>
                ...
                {:else}
                    <div class="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
                        <Package class="w-12 h-12 opacity-20" aria-hidden="true" />
