        ...
        <div class="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800" transition:scale={{ duration: 300, start: 0.95, opacity: 0 }}>
            ...
            <form method="POST" action="?/registerAssetMovement" class="p-8 space-y-6" use:enhance={() => { 
                ...
                <div class="flex gap-3">
                    <button type="button" onclick={() => isOpen = false} class="flex-grow py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-slate-200 transition-colors">Cancelar</button>
                    <button type="submit" disabled={isSubmitting} class="flex-[2] py-4 bg-emerald-600 text-white font-black uppercase tracking-widest text-[10px] rounded-2xl flex items-center justify-center gap-3 shadow-xl shadow-emerald-600/20 hover:bg-emerald-700 transition-colors disabled:opacity-50">
                        {#if isSubmitting}
