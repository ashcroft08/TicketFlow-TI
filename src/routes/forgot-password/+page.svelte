    ...
    <main class="relative w-full max-w-[420px] px-4 sm:px-6 z-10 animate-fade-in-up">
        <div class="bg-white/80 backdrop-blur-xl border border-white/50 rounded-[24px] p-8 sm:p-10 flex flex-col gap-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
            ...
            <header class="flex flex-col items-center gap-2 text-center">
                ...
                <div>
                    <h2 class="font-h2 text-xl sm:text-2xl text-on-surface font-bold tracking-tight">Recuperar Contraseña</h2>
                    <p class="text-xs sm:text-sm text-on-surface-variant mt-1">Enviaremos un enlace de recuperación a tu correo</p>
                </div>
            </header>
            ...
            {#if form?.success}
                <div class="bg-secondary/10 text-secondary border border-secondary/20 p-3.5 rounded-xl text-center text-sm font-medium">
                    {form.message}
                </div>
                <a href="/" class="text-center text-sm font-semibold text-primary hover:underline flex items-center justify-center gap-2">
                    <ArrowLeft size={16} /> Volver al Login
                </a>
            {:else}
                <form method="POST" class="flex flex-col gap-6" onsubmit={() => loading = true}>
                    <div class="flex flex-col gap-2 group">
                        <label class="text-[11px] font-bold text-on-surface-variant uppercase tracking-wider transition-colors group-focus-within:text-primary" for="identifier">Usuario o Correo</label>
                        <div class="relative">
                            <div class="absolute left-3.5 top-1/2 -translate-y-1/2 flex items-center justify-center text-outline/70">
                                <User size={18} />
                            </div>
                            <input
                                class="w-full h-12 pl-11 pr-4 bg-white/50 border border-outline-variant/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm text-on-surface transition-all duration-200"
                    ...
                    <a href="/" class="text-center text-sm font-semibold text-on-surface-variant hover:text-primary transition-colors flex items-center justify-center gap-2 mt-2">
